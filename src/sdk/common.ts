...
import { readableStreamToString, PasbyError, parseIfJson } from "./error";
import { requestAfterHook } from "./components/requestAfterHook";
import { requestBeforeUrlHook } from "./components/requestBeforeUrlHook";
 ...
 * @export
 */
export const setJwtToObject = async function (object: any, configuration?: Configuration) {
    if (configuration && configuration.accessToken) {
        const accessToken = typeof configuration.accessToken === 'function'
            ? await configuration.accessToken()
            : await configuration.accessToken;
        object["x-access-token"] = accessToken;
    }
}
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new PasbyError(e, parseIfJson(responseBody))
            } catch (innerError) {
